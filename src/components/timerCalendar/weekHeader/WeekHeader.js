...
class WeekHeader extends Component {
  ...
  checkCurrentDay = (days) => {
    ...
    days.map((day, index) => {
      if (
        day.getFullYear() === curr.getFullYear() &&
        day.getMonth() === curr.getMonth() &&
        day.getDate() === curr.getDate()
      ) {
        result = index;
      }
    });
  ...
  componentDidMount = () => {
    ...
    this.props.fetchTotalTimeCurrentDay();
    this.props.fetchTotalTimeCurrentWeek();
  };

...
class CountUser extends React.Component {
    ...
    componentDidMount() {
        ...
        if(!this.props.rowStatus) {
            timeCloudAPI().get(`tasks/${this.props.taskId}/users/${this.props.ele.id}/total-times`)
            .then(response => {
                if(this._isMounted) {
                    this.setState({
    ...
    render() {
        ...
        if(rowStatus) {
            if(index > 2) {
                ...
                return <div>
                            <img key={index} alt="" src = {ele.avatar ? ele.avatar : (ele.gender ? male : female)} />
                        </div>
            }
        } else {
            return <div className = "count_user_item">
                        ...
                        <div className = "count_user_item__right">
                            {this.state.tracked}
                        </div>
                    </div>
        }
